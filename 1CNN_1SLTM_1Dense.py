...
class MotorFaultDetector:
    ...
    def build_model(self, sequence_length):
        ...
        model = tf.keras.Sequential(
            [
                ...
                ),  # Wyjście binarne (0 - zdrowy, 1 - uszkodzony)
            ]
        )
    ...
    def preprocess_data(self, data_df, sequence_length=100):
        ...
        for index in range(len(features)):
            ...
            scaled_data[:, index] = (
                self.scalers[current_feature]
                .fit_transform(data_df[current_feature].values.reshape(-1, 1))
                .ravel()
            )
    ...
    def train(self, X_train, X_test, y_train, y_test, epochs=15, batch_size=32):
        ...
        callbacks = [
            ReduceLROnPlateau(
                monitor="val_accuracy", factor=0.2, patience=10, min_lr=1e-6, verbose=1
            ),
            ModelCheckpoint(
